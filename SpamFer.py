...
''' + bcolors.ENDC + '--------------------------------------------------------------')
try:
	server = raw_input(bcolors.OKGREEN + 'Servicio: ' + bcolors.ENDC)
	user = raw_input(bcolors.OKGREEN + 'Tu correo: ' + bcolors.ENDC)
	pwd = getpass.getpass(bcolors.OKGREEN + 'Contraseña: ' + bcolors.ENDC)
	to = raw_input(bcolors.OKGREEN + 'To: ' + bcolors.ENDC)
	subject = raw_input(bcolors.OKGREEN + 'Tema del mensaje: ' + bcolors.ENDC)
	body = raw_input(bcolors.OKGREEN + 'Mensaje: ' + bcolors.ENDC)
	nomes = input(bcolors.OKGREEN + 'Numero de correos a enviar: ' + bcolors.ENDC)
	no = 0
	message = 'From: ' + user + '\nSubject: ' + subject + '\n' + body
...
if server == '1' or server == 'gmail' or server == 'Gmail':
	...
	try:
		...
		print bcolors.FAIL + '''Su correo o contraseña esta mal escrita, escribelo nuevamente.... ''' + bcolors.ENDC
		sys.exit()
	while no != nomes:
		try:
